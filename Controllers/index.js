...
const createRoom = (req, res) => {
  ...
  if (rooms.find((room) => room.roomNumber === roomNumber)) {
    return res
      .status(400)
      .send({ error: "Room with the given number already exists" });
  }
  const newroom = {
    roomNumber,
    seats,
...
const bookRooms = (req, res) => {
  const { bookingId, customerName, date, startTime, endTime, roomId } =
    req.body;
  ...
  if (overlapBooking) {
    return res
      .status(400)
      .send({ error: "Room already booked for the time slot" });
  }
